...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (!rev_packet(buf, (size_t)n, &total_frag, &frag_no, &size, filename, &data_start)) {
        ...
        if (drop_pkt()){
            ...
            continue; //wait for client's timeout
        }
